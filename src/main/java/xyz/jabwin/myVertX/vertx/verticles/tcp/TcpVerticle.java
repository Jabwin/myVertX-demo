...
public class TcpVerticle extends AbstractVerticle
{
    ...
    public void start()
    {
        vertx.createNetServer()
            ...
            .listen(7777)
            .onSuccess(this::netServerSuccessHandler);
    }
    ...
    private void socketConnectHandler(NetSocket socket)
    {
        socket.handler(buffer ->
        {
            TCPMsg tcpMsg = new TCPMsg()
                    ...
                    .setSocket(socket);
            if (socketMap.containsValue(socket)) eventBus.request("/tcp", tcpMsg);
            else eventBus.request("/tcp/auth", tcpMsg);
        });
    ...
    /** tcp端口监听成功回调 */
    private void netServerSuccessHandler(NetServer server)
    {
